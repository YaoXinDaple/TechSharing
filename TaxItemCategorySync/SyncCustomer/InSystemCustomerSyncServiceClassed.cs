...
{
    public class InSystemCustomerSyncServiceClassed
    {
        /// <summary>
        ...
        public InSystemCustomerSyncServiceClassed(LemonInvoiceSuiteContext dbcontext)
        {
            //创建同步客户信息的Channel
            var option = new BoundedChannelOptions(ChannelCapacity)
            {
        ...
        private async Task WriteCustomersSyncMessageTask(ChannelWriter<SaveSyncCustomerMessage> writer, Guid companyId)
        {
            ...
            while (shouldFetchNextBatch)
            {
                ...
                {
                    var customer = new CustomerSyncItem(info.CustomerName, info.CustomerUscic);
                    SaveSyncCustomerMessage writeMessage;
                    customerNamesToSync.Add(customer);

                    writeMessage = new SaveSyncCustomerMessage(companyId, info.ArchivedInvoiceId, customer);
        ...
        private sealed record CustomerSyncItem(string Name, string? Uscic);
        ...
        private sealed record SaveSyncCustomerMessage(Guid CompanyId, Guid ArchivedInvoiceId, CustomerSyncItem? Item);
